...
#'
#' Calculates the volumetric water content (cm3/cm3)
...
#' #' @examples
#' temp1 <- MWD(WC = c(4.237,42.42,36.326,20.34),ADC=c(5,1.1265,0.153,0.0265));
#' @export
# 17. Mean weight diameter
MWD<- function(WC,ADC){
  total_WC <- sum(WC)
  WC1 <- WC/total_WC
  product <- WC1*ADC
...
#' #' @examples
#' temp1 <- GMD(WC = c(4.237,42.42,36.326,20.34),ADC=c(5,1.1265,0.153,0.0265));
#' @export
# 18. Geometric mean diameter
GMD<- function(WC,ADC){
  total_WC <- sum(WC)
  WC1 <- WC/total_WC
...
#' all_soil_indices <- Soil_Indices(file_path);
#' Soil_Indices( );
#' @export
#
